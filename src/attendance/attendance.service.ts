...
export class AttendanceService {
  ...
  async record(companyId: string, userId: string, input: CreateAttendanceDto) {
    ...
    if (!worker || !membership) throw new NotFoundException('Travailleur, chantier ou affectation introuvable.');
  ...
  async summary(companyId: string, userId: string, chantierId: string, from?: string, to?: string) {
    ...
    for (const attendance of attendances) {
      ...
      if (attendance.status === AttendanceStatus.RETARD) late += 1;
      if (attendance.status !== AttendanceStatus.ABSENT) laborCost = laborCost.plus(attendance.worker.dailyRate ?? 0);
    }
